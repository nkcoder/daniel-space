...
- Deploy, scale and manage a fleet of 3rd party network virtual appliances in AWS, for example: - Firewall - Intrusion Detection and Prevention Systems
- Support protocols: HTTP1, HTTP2 and gRPC (configured in _Target Groups_)
...
- Can scale an ASG based on CloudWatch alarms
  - metrics such as _Average CPU_ are computed for the overall ASG instances
- Scaling policies
  - dynamic scaling
  ...
  - **Note**: _predictive scaling policies_ and _scheduled actions_ are not available when you create the ASG, so you should add these two policies after the ASG is created.
