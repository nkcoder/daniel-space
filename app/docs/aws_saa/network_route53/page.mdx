...
- For example: we have a ALB: my-backend.ap-southeast-2.elb.amazonaws.com, the following two configs are the same - CNAME: _my-backend.my-domain.com_ => _my-backend.ap-southeast-2.elb.amazonaws.com_ - A (Alias): _my-backend.my-domain.com_ => _my-backend.ap-southeast-2.elb.amazonaws.com_
