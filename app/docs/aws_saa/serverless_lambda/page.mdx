...
- by default, the environment variables are encrypted using the default KMS key for lambda (*aws/lambda*); when the lambda is invoked, those values are decrypted and make available to the code
