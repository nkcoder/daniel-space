...
- When you upload an object to a bucket in the AWS console, you can configure the *Permissions (ACL)* and *Properties (storage class, object lock, server side encryption)*. You can make the object public accessible via ACL only when Block public access and Object Ownership are disabled.
